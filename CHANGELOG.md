...
- **Pictures for the account, an artist and a song.** Account settings, an artist's page and a song's settings each take an image (JPEG, PNG or WebP, shrunk in the browser to 1024 px before upload, so a phone photo is a few hundred KB); it shows on the projects page header, the artist's page and the directory (round for a solo artist), and beside the song's title. Migration 0052 adds `image_url` to `account`, `artist` and `song`.
