...
- **A Demos view on the song page.** The player's box shows either the stems or the demo recordings, switched by Stems / Demos tabs above it (and the Demos entry in the Downloads menu); a song with demos but no stems opens on the demos. The demos view (`DemoPanel.svelte`) has a transport with previous, play/pause, next, a position slider and volume over the list of demos, each with its own play and Download buttons, and Upload a demo / Record a demo for editors. It replaces the Demos popover and the Demos button in the action row. At xl the Stems / Demos tabs always show, and the documents' tool bar moves into a row above its panel so the two boxes line up.
