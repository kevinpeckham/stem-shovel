...
/**
 * Pictures for an account, an artist or a project. The browser resizes before
 * sending (src/lib/utils/resizeImage.ts); the server stores the file in
 * Blob under the account (a project's in the private store when the project
 * is private), points the row at it, and deletes the one before.
 */

async function editorOf(kind: ImageKind, id: string) {
	const { locals } = getRequestEvent();
	if (kind === "account") return requireEditor(locals, id);
	return memberOf(locals, kind === "artist" ? accountOfArtist : accountOfProject, id);
}
...
export const setImage = form(ImageSetSchema, async ({ kind, id, image }) => {
	...
	const access =
		kind === "project" && (await projectIsPrivate(accountId, id)) ? "private" : "public";
